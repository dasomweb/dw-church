...
class DW_Church_Loader {
    ...
    public function dasom_church_save_post_meta($post_id) {
        ...
        if ($post_type === 'dasom_bulletin') {
            ...
            if ($date) {
                $new_title = date_i18n(__('Y??n??j??, 'dw-church'), strtotime($date)) . ' ' . __('Church Bulletin', 'dw-church');
    ...
    public function dasom_church_admin_scripts($hook) {
        ...
        if (($hook == 'post-new.php' || $hook == 'post.php') && isset($post->post_type)) {
            ...
            if (in_array($post->post_type, $post_types)) {
                wp_enqueue_script('dw-church-admin', DASOM_CHURCH_PLUGIN_URL . 'assets/js/admin.js', array('jquery', 'wp-media'), DASOM_CHURCH_VERSION, true);
