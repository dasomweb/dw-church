...
const BLOCK_ICON_PATHS: Record<string, string> = {
  ...
  featured_event: 'M4 5h16v15H4zM4 9h16M8 3v4M16 3v4M11 14l1.5 3 3 .4-2.2 2.1.5 3-2.8-1.5-2.8 1.5.5-3L6.5 17.4l3-.4z',
  verse_of_day: 'M4 5a2 2 0 012-2h7v16H6a2 2 0 00-2 2zM20 5a2 2 0 00-2-2h-1v16h1a2 2 0 012 2zM9 7h1M9 10h1',
  sermon_meditation: 'M4 5h9v14H6a2 2 0 01-2-2zM4 5a2 2 0 012-2h7M16 5h4v14h-4zM16 9h4M16 13h4',
  news_announcements: 'M4 6h16v12a2 2 0 01-2 2H6a2 2 0 01-2-2zM4 6L4 4h11v2M8 10h8M8 13h8M8 16h5',
  ...
  page_subnav: 'M4 5h16v8H4zM6 17h4M11 17h7',
  online_bulletin: 'M6 3h9l3 3v15H6zM9 8h6M9 12h6M9 16h4',
  prose_image: 'M4 5h10M4 8h16M5 12h14v7H5z',
  values_grid: 'M3 6h5v12H3zM10 6h4v12h-4zM16 6h5v12h-5z',
  detail_rows: 'M4 6h16v4H4zM10 6v4M4 14h16v4H4zM10 14v4',
  info_bar: 'M3 9h18v6H3zM9 9v6M15 9v6',
...
export interface BlockDef {
  ...
  editableFields: { key: string; label: string; type: 'text' | 'textarea' | 'richtext' | 'number' | 'select' | 'image' | 'images' | 'url' | 'array' | 'tags' | 'services' | 'buttons' | 'groups' | 'cardItems' | 'board-select'; options?: { label: string; value: string }[]; max?: number; withImage?: boolean; multi?: boolean }[];
}
...
export const BLOCK_DEFS: BlockDef[] = [
  // ─── Hero ─────────────────────────────────────
  { type: 'hero_banner', label: '히어로 배너', category: '히어로', icon: '🖼️', nature: 'static', description: '배경 이미지 + 텍스트 오버레이 배너', variants: [{ id: 'centered', label: '중앙' }, { id: 'left', label: '좌측' }, { id: 'masthead', label: '마스트헤드(신문형·이미지 없음)' }], defaultProps: { title: '환영합니다', subtitle: '', height: 'md', textAlign: 'center', layout: 'full', overlayColor: '#000000', overlayOpacity: 50 }, editableFields: [{ key: 'title', label: '제목', type: 'text' }, { key: 'subtitle', label: '부제목', type: 'text' }, { key: 'eyebrow', label: '윗글(영문 라벨 등)', type: 'text' }, { key: 'meta', label: '마스트헤드 하단 줄(예: 날짜)', type: 'text' }, { key: 'backgroundImageUrl', label: '배경 이미지', type: 'image' }, { key: 'backgroundImagePosition', label: '배경 이미지 위치', type: 'select', options: [{ label: '가운데 (기본)', value: 'center' }, { label: '상단 중앙', value: 'top' }, { label: '하단 중앙', value: 'bottom' }, { label: '좌측 중앙', value: 'left' }, { label: '우측 중앙', value: 'right' }, { label: '좌측 상단', value: 'top-left' }, { label: '우측 상단', value: 'top-right' }, { label: '좌측 하단', value: 'bottom-left' }, { label: '우측 하단', value: 'bottom-right' }] }, { key: 'overlayColor', label: '오버레이 색상', type: 'text' }, { key: 'overlayOpacity', label: '오버레이 투명도 (%)', type: 'number' }, { key: 'buttonText', label: '버튼 텍스트', type: 'text' }, { key: 'buttonUrl', label: '버튼 링크', type: 'url' }, { key: 'layout', label: '레이아웃', type: 'select', options: [{ label: '풀 와이드', value: 'full' }, { label: '컨테이너', value: 'contained' }] }, { key: 'height', label: '높이', type: 'select', options: [{ label: '작게', value: 'sm' }, { label: '보통', value: 'md' }, { label: '크게', value: 'lg' }, { label: '전체', value: 'full' }] }] },
  ...
  { type: 'values_grid', label: '가치 카드 그리드', category: '소개', icon: '💎', nature: 'static', description: '굵은 밑줄 제목 + N열 카드 (오버라인 + 제목 + 설명)', variants: [{ id: 'columns-3', label: '3열' }, { id: 'columns-2', label: '2열' }, { id: 'columns-4', label: '4열' }], defaultProps: { title: '', columns: '3', showRule: true, items: [] }, editableFields: [{ key: 'title', label: '제목', type: 'text' }, { key: 'columns', label: '열 수', type: 'select', options: [{ label: '2열', value: '2' }, { label: '3열', value: '3' }, { label: '4열', value: '4' }] }, { key: 'showRule', label: '제목 밑줄', type: 'select', options: [{ label: '표시', value: 'true' }, { label: '숨김', value: 'false' }] }, { key: 'items', label: '카드 항목 (제목=카드 제목, 내용=설명 · 오버라인은 고급 편집기)', type: 'cardItems', withImage: false }] },
  { type: 'detail_rows', label: '항목 리스트 (제목+설명)', category: '소개', icon: '🧾', nature: 'static', description: '굵은 밑줄 제목 + 도입문 + 행 리스트 (좌: 제목/라벨, 우: 설명/보조문)', variants: [], defaultProps: { title: '', intro: '', showRule: true, items: [] }, editableFields: [{ key: 'title', label: '제목', type: 'text' }, { key: 'intro', label: '도입문', type: 'richtext' }, { key: 'showRule', label: '제목 밑줄', type: 'select', options: [{ label: '표시', value: 'true' }, { label: '숨김', value: 'false' }] }, { key: 'items', label: '행 항목 (제목=좌측 제목, 내용=우측 설명 · 라벨/보조문은 고급 편집기)', type: 'cardItems', withImage: false }] },
  { type: 'info_bar', label: '정보 바 (예배시간)', category: '소개', icon: '🎗️', nature: 'static', description: '컬러 밴드에 라벨+값 셀 N개를 가로로 (예배시간 바·빠른 정보)', variants: [], defaultProps: { items: [], background: 'primary', align: 'left' }, editableFields: [{ key: 'background', label: '밴드 배경 (palette key/hex, 기본 primary)', type: 'text' }, { key: 'align', label: '정렬', type: 'select', options: [{ label: '좌측', value: 'left' }, { label: '가운데', value: 'center' }] }, { key: 'items', label: '셀 (제목=라벨, 내용=값)', type: 'cardItems', withImage: false }] },
  { type: 'info_columns', label: '정보 컬럼 (한눈에)', category: '소개', icon: '📋', nature: 'static', description: '예배시간·오시는 길 등 2~4칸 정보 그리드 (라벨 + 값)', variants: [{ id: 'columns-3', label: '3칸' }, { id: 'columns-2', label: '2칸' }, { id: 'columns-4', label: '4칸' }], defaultProps: { title: '', columns: '3', items: [] }, editableFields: [{ key: 'eyebrow', label: '라벨(윗글)', type: 'text' }, { key: 'title', label: '제목', type: 'text' }, { key: 'columns', label: '칸 수', type: 'select', options: [{ label: '2칸', value: '2' }, { label: '3칸', value: '3' }, { label: '4칸', value: '4' }] }, { key: 'items', label: '정보 칸 (제목=라벨, 내용=값)', type: 'cardItems', withImage: false }] },
  { type: 'week_schedule', label: '이번 주 일정', category: '소개', icon: '🗓️', nature: 'static', description: '요일 배지 + 내용 한 줄의 주간 일정 아젠다 리스트', variants: [], defaultProps: { title: '이번 주 교회는', items: [] }, editableFields: [{ key: 'eyebrow', label: '라벨(윗글)', type: 'text' }, { key: 'title', label: '제목', type: 'text' }, { key: 'items', label: '일정 행 (제목=요일, 내용=모임·시간·장소)', type: 'cardItems', withImage: false }] },
  { type: 'hero_overlap', label: '히어로 + 겹침 안내카드', category: '히어로', icon: '🪧', nature: 'static', description: '히어로 사진 위 제목 + 아래로 겹치는 예배 안내 3칸 카드', variants: [], defaultProps: { eyebrow: '', title: '', subtitle: '', backgroundImageUrl: '', cards: [] }, editableFields: [{ key: 'eyebrow', label: '라벨(윗글)', type: 'text' }, { key: 'title', label: '제목', type: 'text' }, { key: 'subtitle', label: '부제', type: 'text' }, { key: 'backgroundImageUrl', label: '배경 이미지', type: 'image' }] },
  { type: 'sermon_feature', label: '설교 피처드', category: '콘텐츠', icon: '🎬', nature: 'static', description: '큰 설교 카드 + 옆 최근 설교 목록', variants: [], defaultProps: { eyebrow: '설교 말씀', title: '이번 주 말씀', featured: {}, items: [] }, editableFields: [{ key: 'eyebrow', label: '라벨(윗글)', type: 'text' }, { key: 'title', label: '제목', type: 'text' }, { key: 'moreLabel', label: '더보기 링크', type: 'text' }, { key: 'footerLabel', label: '하단 버튼', type: 'text' }] },
  { type: 'news_split', label: '소식 + 나눔 카드 (2단)', category: '콘텐츠', icon: '📰', nature: 'static', description: '소식/주보 리스트 + 옆 나눔·바로가기 카드', variants: [], defaultProps: { title: '주보 · 공지', items: [], sideTitle: '', buttons: [], links: [] }, editableFields: [{ key: 'title', label: '제목', type: 'text' }, { key: 'sideTitle', label: '우측 카드 제목', type: 'text' }, { key: 'sideDesc', label: '우측 안내문', type: 'textarea' }] },
  { type: 'bento_grid', label: '매거진 벤토 그리드', category: '히어로', icon: '🧱', nature: 'static', description: '사진·카드·리스트를 크기별로 배치하는 매거진형 홈 (모바일 1칸)', variants: [], defaultProps: { columns: 6, tiles: [] }, editableFields: [] },
  { type: 'dashboard_banner', label: '대시보드 환영 배너', category: '히어로', icon: '📋', nature: 'static', description: '브랜드 컬러 라운드 배너: 날짜 + 제목 + 안내 + pill 버튼 2개 (시안 15)', variants: [], defaultProps: { eyebrow: '', title: '이번 주 예배 안내', subtitle: '', buttonText: '주보 보기', buttonUrl: '/bulletins', secondaryButtonText: '생중계', secondaryButtonUrl: '#' }, editableFields: [{ key: 'eyebrow', label: '날짜/라벨(윗글)', type: 'text' }, { key: 'title', label: '제목', type: 'text' }, { key: 'subtitle', label: '안내 문구', type: 'text' }, { key: 'buttonText', label: '버튼1 텍스트', type: 'text' }, { key: 'buttonUrl', label: '버튼1 링크', type: 'url' }, { key: 'secondaryButtonText', label: '버튼2 텍스트', type: 'text' }, { key: 'secondaryButtonUrl', label: '버튼2 링크', type: 'url' }] },

  // ─── Content ───────────────────────────────────
  { type: 'recent_sermons', label: '설교', category: '콘텐츠', icon: '🎤', nature: 'dynamic', description: '최근 설교 목록', variants: [{ id: 'grid-4', label: '4열' }, { id: 'grid-3', label: '3열' }, { id: 'grid-2', label: '2열' }, { id: 'list', label: '리스트' }, { id: 'featured', label: '피처드(대표+목록)' }, { id: 'card', label: '단일 카드(영상 썸네일)' }], defaultProps: { limit: 6 }, editableFields: [{ key: 'title', label: '제목', type: 'text' }, { key: 'limit', label: '표시 개수', type: 'number' }] },
  { type: 'sermon_magazine', label: '설교 매거진', category: '콘텐츠', icon: '📖', nature: 'dynamic', description: '이번 주 설교 — 한줄요약·써머리·질문(관찰·심화·적용)', variants: [{ id: 'default', label: '매거진(영상)' }, { id: 'compact', label: '간결형(영상 없이)' }], defaultProps: { title: '이번 주 말씀' }, editableFields: [{ key: 'title', label: '상단 라벨', type: 'text' }, { key: 'variant', label: '레이아웃', type: 'select', options: [{ label: '매거진(영상 썸네일)', value: 'default' }, { label: '간결형(영상 없이·개척교회)', value: 'compact' }] }] },
  { type: 'sermon_meditation', label: '주일설교 묵상', category: '콘텐츠', icon: '📖', nature: 'dynamic', description: '대표 설교(설교 모듈 최신) + 이번 주 묵상 사이드바(말씀·책·기도제목·주보) — 에디토리얼 홈용', variants: [], defaultProps: { eyebrow: '주일설교', asideTitle: '이번 주 묵상', moreLabel: '설교 전문 읽기', bulletinLabel: '이번 주 주보', bulletinUrl: '/onlinejubo', showAside: true }, editableFields: [{ key: 'eyebrow', label: '라벨(윗글)', type: 'text' }, { key: 'moreLabel', label: '설교 링크 텍스트', type: 'text' }, { key: 'asideTitle', label: '사이드바 제목', type: 'text' }, { key: 'verseText', label: '이번 주 말씀 (비우면 말씀 모듈 자동)', type: 'textarea' }, { key: 'verseRef', label: '말씀 출처', type: 'text' }, { key: 'bookTitle', label: '함께 읽는 책 제목', type: 'text' }, { key: 'bookMeta', label: '책 저자·범위', type: 'text' }, { key: 'prayerText', label: '기도 제목 (줄바꿈으로 구분)', type: 'textarea' }, { key: 'bulletinLabel', label: '주보 링크 텍스트', type: 'text' }, { key: 'bulletinUrl', label: '주보 링크 주소', type: 'text' }] },
  { type: 'devotion_reader', label: '말씀 묵상 (QT)', category: '콘텐츠', icon: '📖', nature: 'dynamic', description: '주간 묵상 목록 + 오늘 묵상 (읽기 전용, 관리자 업로드)', variants: [{ id: 'default', label: '리더' }, { id: 'teaser', label: '진입 카드(홈용)' }], defaultProps: { title: '말씀 묵상', eyebrow: '이번 주 묵상' }, editableFields: [{ key: 'variant', label: '레이아웃', type: 'select', options: [{ label: '리더(주간 목록+오늘 묵상)', value: 'default' }, { label: '진입 카드(홈용)', value: 'teaser' }] }, { key: 'title', label: '제목', type: 'text' }, { key: 'eyebrow', label: '좌측 라벨 / 소제목', type: 'text' }, { key: 'description', label: '소개 문구(진입 카드)', type: 'textarea' }, { key: 'imageUrl', label: '이미지(진입 카드)', type: 'image' }] },
  { type: 'cardnews', label: '카드뉴스', category: '콘텐츠', icon: '🗞️', nature: 'dynamic', description: '여러 장 카드로 넘겨 보는 소식(카드뉴스 관리에서 업로드). 표지를 누르면 뷰어가 열림', variants: [{ id: 'carousel', label: '캐러셀' }, { id: 'grid', label: '그리드' }, { id: 'rail', label: '가로 레일' }], defaultProps: { variant: 'carousel', eyebrow: '카드뉴스', title: '한 장으로 보내는 소식', items: [] }, editableFields: [{ key: 'eyebrow', label: '라벨(윗글)', type: 'text' }, { key: 'title', label: '제목', type: 'text' }, { key: 'items', label: '(선택) 정적 카드 — 카드뉴스 관리에 등록하면 자동 표시', type: 'cardItems', withImage: true }] },
  ...
  { type: 'online_bulletin', label: '온라인 주보', category: '콘텐츠', icon: '📜', nature: 'dynamic', description: '온라인 주보 관리의 최신 발행분을 스크롤로 전체 표시 (예배순서·찬양악보·성경본문·소그룹 질문 등)', variants: [], defaultProps: {}, editableFields: [{ key: 'bulletinId', label: '특정 주보 ID (비우면 최신 발행분 자동)', type: 'text' }] },
  ...
  { type: 'featured_event', label: '다가오는 행사(알림바)', category: '콘텐츠', icon: '📣', nature: 'dynamic', description: '행사 1개를 골라 상단 알림바로 노출 · 미선택/종료일 경과 시 자동 숨김', variants: [], defaultProps: { label: '다가오는 행사', buttonText: '자세히 보기' }, editableFields: [{ key: 'label', label: '라벨(배지)', type: 'text' }, { key: 'buttonText', label: '버튼 텍스트', type: 'text' }] },
  { type: 'verse_of_day', label: '오늘의 말씀', category: '콘텐츠', icon: '📖', nature: 'dynamic', description: '말씀 관리에 등록한 현재 말씀 1개를 표시 · 없으면 자동 숨김', variants: [], defaultProps: { eyebrow: '오늘의 말씀 · Verse of the Day' }, editableFields: [{ key: 'eyebrow', label: '라벨(윗글)', type: 'text' }] },
  { type: 'news_announcements', label: '주보·광고', category: '콘텐츠', icon: '🗞️', nature: 'dynamic', description: '주보(주보 모듈) + 광고(교회소식 게시판, 카테고리 배지) 목록 + 원하는 만큼 추가하는 버튼', variants: [], defaultProps: { title: '주보 · 광고', newsLimit: 4, moreUrl: '/bulletins', buttons: [{ text: '기도 요청', url: '/contact' }, { text: '심방 신청', url: '/contact' }] }, editableFields: [{ key: 'title', label: '제목', type: 'text' }, { key: 'boardSlug', label: '교회소식 게시판', type: 'board-select' }, { key: 'newsLimit', label: '광고 개수', type: 'number' }, { key: 'buttons', label: '버튼', type: 'buttons' }] },
  ...
  { type: 'video_board', label: '영상 게시판', category: '콘텐츠', icon: '📺', nature: 'dynamic', description: '카테고리별 유튜브 영상', variants: [{ id: 'grid-1', label: '1열(크게)' }, { id: 'grid-2', label: '2열' }, { id: 'grid-3', label: '3열' }, { id: 'grid-4', label: '4열' }], defaultProps: { title: '영상', category: '', limit: 6 }, editableFields: [{ key: 'title', label: '제목', type: 'text' }, { key: 'category', label: '카테고리 (이름 또는 슬러그)', type: 'text' }, { key: 'limit', label: '표시 개수', type: 'number' }] },
  ...
  { type: 'board', label: '게시판', category: '콘텐츠', icon: '📋', nature: 'dynamic', description: '게시판 (하나 또는 여러 개 선택)', variants: [{ id: 'list', label: '리스트' }, { id: 'grid-2', label: '2열' }, { id: 'grid-3', label: '3열' }], defaultProps: { boardSlugs: [], limit: 10 }, editableFields: [{ key: 'title', label: '제목', type: 'text' }, { key: 'boardSlugs', label: '게시판 선택 (여러 개 가능)', type: 'board-select', multi: true }, { key: 'limit', label: '표시 개수', type: 'number' }] },
  ...
  { type: 'custom_form', label: '커스텀 폼', category: '폼/신청서', icon: '🧩', nature: 'dynamic', description: "'폼 만들기'에서 설계한 폼을 slug로 불러와 표시 → 폼 제출 인박스", variants: [], defaultProps: { formSlug: '' }, editableFields: [{ key: 'formSlug', label: '폼 slug (폼 만들기에서 생성)', type: 'text' }, { key: 'title', label: '제목(선택)', type: 'text' }, { key: 'subtitle', label: '안내 문구(선택)', type: 'textarea' }] },
  // FormSplitBlock reads props.layout (form-left/form-right), NOT props.variant,
  // so expose layout as a select field instead of a variant toggle.
  { type: 'form_split', label: '폼 + 텍스트 (2단)', category: '폼/신청서', icon: '📝', nature: 'dynamic', description: "폼과 설명 텍스트를 좌우 2단으로 배치 ('폼 만들기'의 폼을 slug로 불러옴)", variants: [], defaultProps: { formSlug: '', layout: 'form-left', title: '문의하기', subtitle: '', content: '' }, editableFields: [{ key: 'formSlug', label: '폼 slug (폼 만들기에서 생성)', type: 'text' }, { key: 'layout', label: '배치', type: 'select', options: [{ label: '폼 왼쪽 · 텍스트 오른쪽', value: 'form-left' }, { label: '텍스트 왼쪽 · 폼 오른쪽', value: 'form-right' }] }, { key: 'title', label: '제목', type: 'text' }, { key: 'subtitle', label: '부제목', type: 'text' }, { key: 'content', label: '옆에 표시할 텍스트', type: 'textarea' }] },

  // ─── Text ──────────────────────────────────────
  { type: 'text_image', label: '텍스트+이미지', category: '텍스트', icon: '📝', nature: 'static', description: '텍스트와 이미지', variants: [{ id: 'right', label: '이미지 우측' }, { id: 'left', label: '이미지 좌측' }], defaultProps: { content: '', imageUrl: '' }, editableFields: [{ key: 'title', label: '제목', type: 'text' }, { key: 'content', label: '내용', type: 'richtext' }, { key: 'imageUrl', label: '이미지', type: 'image' }] },
  { type: 'text_only', label: '텍스트', category: '텍스트', icon: '📃', nature: 'static', description: '텍스트 전용', variants: [{ id: 'left', label: '좌측 정렬' }, { id: 'center', label: '중앙 정렬' }], defaultProps: { content: '' }, editableFields: [{ key: 'title', label: '제목', type: 'text' }, { key: 'content', label: '내용', type: 'richtext' }] },
  { type: 'prose_image', label: '제목+본문+이미지 (세로)', category: '텍스트', icon: '🖼️', nature: 'static', description: '굵은 밑줄 제목 + 본문 + 전체폭 이미지 (한 컬럼 읽기 레이아웃)', variants: [], defaultProps: { title: '', body: '', imageUrl: '', imagePosition: 'below', showRule: true }, editableFields: [{ key: 'eyebrow', label: '라벨(윗글)', type: 'text' }, { key: 'title', label: '제목', type: 'text' }, { key: 'body', label: '본문', type: 'richtext' }, { key: 'imageUrl', label: '이미지', type: 'image' }, { key: 'imagePosition', label: '이미지 위치', type: 'select', options: [{ label: '본문 아래', value: 'below' }, { label: '본문 위', value: 'above' }] }, { key: 'showRule', label: '제목 밑줄', type: 'select', options: [{ label: '표시', value: 'true' }, { label: '숨김', value: 'false' }] }] },
  { type: 'quote_block', label: '인용/성경구절', category: '텍스트', icon: '✝️', nature: 'static', description: '인용문 또는 성경 말씀', variants: [{ id: 'card', label: '카드' }, { id: 'simple', label: '심플' }, { id: 'highlight', label: '하이라이트' }, { id: 'verse', label: '말씀 콜아웃' }], defaultProps: { quote: '' }, editableFields: [{ key: 'eyebrow', label: '라벨(윗글)', type: 'text' }, { key: 'quote', label: '인용문', type: 'textarea' }, { key: 'source', label: '출처', type: 'text' }, { key: 'reference', label: '참조', type: 'text' }, { key: 'backgroundImageUrl', label: '배경 이미지', type: 'image' }, { key: 'backgroundImagePosition', label: '배경 이미지 위치', type: 'select', options: [{ label: '가운데 (기본)', value: 'center' }, { label: '상단 중앙', value: 'top' }, { label: '하단 중앙', value: 'bottom' }, { label: '좌측 중앙', value: 'left' }, { label: '우측 중앙', value: 'right' }, { label: '좌측 상단', value: 'top-left' }, { label: '우측 상단', value: 'top-right' }, { label: '좌측 하단', value: 'bottom-left' }, { label: '우측 하단', value: 'bottom-right' }] }] },
  ...
  // ─── CTA ───────────────────────────────────────
  { type: 'call_to_action', label: 'CTA 배너', category: 'CTA', icon: '🔔', nature: 'static', description: '행동 유도 배너', variants: [{ id: 'centered', label: '중앙' }, { id: 'split', label: '분할' }, { id: 'banner', label: '배너' }], defaultProps: { title: '', ctaLabel: '', ctaUrl: '' }, editableFields: [{ key: 'title', label: '제목', type: 'text' }, { key: 'description', label: '설명', type: 'text' }, { key: 'backgroundImageUrl', label: '배경 이미지', type: 'image' }, { key: 'backgroundImagePosition', label: '배경 이미지 위치', type: 'select', options: [{ label: '가운데 (기본)', value: 'center' }, { label: '상단 중앙', value: 'top' }, { label: '하단 중앙', value: 'bottom' }, { label: '좌측 중앙', value: 'left' }, { label: '우측 중앙', value: 'right' }, { label: '좌측 상단', value: 'top-left' }, { label: '우측 상단', value: 'top-right' }, { label: '좌측 하단', value: 'bottom-left' }, { label: '우측 하단', value: 'bottom-right' }] }, { key: 'ctaLabel', label: '버튼 텍스트', type: 'text' }, { key: 'ctaUrl', label: '버튼 링크', type: 'url' }] },
  ...
  // ─── Layout Block ──────────────────────────────
  { type: 'layout_row', label: '행 (Row)', category: '레이아웃', icon: '↔️', nature: 'static', description: '자식 블록을 가로로 배치', variants: [], defaultProps: { layout: 'row', gap: 16, padding: '24px', children: [] }, editableFields: [{ key: 'padding', label: '패딩 (CSS)', type: 'text' }, { key: 'margin', label: '마진 (CSS)', type: 'text' }, { key: 'gap', label: '간격 (px)', type: 'number' }, { key: 'backgroundColor', label: '배경색', type: 'text' }, { key: 'backgroundImageUrl', label: '배경 이미지', type: 'image' }, { key: 'backgroundImagePosition', label: '배경 이미지 위치', type: 'select', options: [{ label: '가운데 (기본)', value: 'center' }, { label: '상단 중앙', value: 'top' }, { label: '하단 중앙', value: 'bottom' }, { label: '좌측 중앙', value: 'left' }, { label: '우측 중앙', value: 'right' }, { label: '좌측 상단', value: 'top-left' }, { label: '우측 상단', value: 'top-right' }, { label: '좌측 하단', value: 'bottom-left' }, { label: '우측 하단', value: 'bottom-right' }] }, { key: 'overlayColor', label: '오버레이 색상', type: 'text' }, { key: 'overlayOpacity', label: '오버레이 투명도 (%)', type: 'number' }, { key: 'borderColor', label: '테두리 색상', type: 'text' }, { key: 'borderWidth', label: '테두리 두께 (px)', type: 'number' }, { key: 'borderRadius', label: '모서리 둥글기 (px)', type: 'number' }, { key: 'divider', label: '구분선 표시', type: 'select', options: [{ label: '없음', value: '' }, { label: '있음', value: 'true' }] }, { key: 'dividerColor', label: '구분선 색상', type: 'text' }, { key: 'linkUrl', label: '링크 URL', type: 'url' }, { key: 'linkTarget', label: '링크 타겟', type: 'select', options: [{ label: '현재 창', value: '_self' }, { label: '새 창', value: '_blank' }] }] },
  { type: 'layout_columns', label: '컬럼 (Columns)', category: '레이아웃', icon: '▥', nature: 'static', description: '2~4열 그리드 레이아웃', variants: [{ id: 'columns-2', label: '2열' }, { id: 'columns-3', label: '3열' }, { id: 'columns-4', label: '4열' }], defaultProps: { layout: 'columns-2', gap: 24, padding: '24px', children: [] }, editableFields: [{ key: 'layout', label: '열 수', type: 'select', options: [{ label: '2열', value: 'columns-2' }, { label: '3열', value: 'columns-3' }, { label: '4열', value: 'columns-4' }] }, { key: 'padding', label: '패딩', type: 'text' }, { key: 'margin', label: '마진', type: 'text' }, { key: 'gap', label: '간격 (px)', type: 'number' }, { key: 'backgroundColor', label: '배경색', type: 'text' }, { key: 'backgroundImageUrl', label: '배경 이미지', type: 'image' }, { key: 'backgroundImagePosition', label: '배경 이미지 위치', type: 'select', options: [{ label: '가운데 (기본)', value: 'center' }, { label: '상단 중앙', value: 'top' }, { label: '하단 중앙', value: 'bottom' }, { label: '좌측 중앙', value: 'left' }, { label: '우측 중앙', value: 'right' }, { label: '좌측 상단', value: 'top-left' }, { label: '우측 상단', value: 'top-right' }, { label: '좌측 하단', value: 'bottom-left' }, { label: '우측 하단', value: 'bottom-right' }] }, { key: 'overlayColor', label: '오버레이 색상', type: 'text' }, { key: 'overlayOpacity', label: '오버레이 투명도 (%)', type: 'number' }, { key: 'borderColor', label: '테두리 색상', type: 'text' }, { key: 'borderWidth', label: '테두리 두께 (px)', type: 'number' }, { key: 'linkUrl', label: '링크 URL', type: 'url' }] },
  { type: 'layout_section', label: '섹션 (Section)', category: '레이아웃', icon: '▣', nature: 'static', description: '배경+패딩 컨테이너', variants: [], defaultProps: { layout: 'section', padding: '40px 24px', children: [] }, editableFields: [{ key: 'padding', label: '패딩', type: 'text' }, { key: 'margin', label: '마진', type: 'text' }, { key: 'backgroundColor', label: '배경색', type: 'text' }, { key: 'backgroundImageUrl', label: '배경 이미지', type: 'image' }, { key: 'backgroundImagePosition', label: '배경 이미지 위치', type: 'select', options: [{ label: '가운데 (기본)', value: 'center' }, { label: '상단 중앙', value: 'top' }, { label: '하단 중앙', value: 'bottom' }, { label: '좌측 중앙', value: 'left' }, { label: '우측 중앙', value: 'right' }, { label: '좌측 상단', value: 'top-left' }, { label: '우측 상단', value: 'top-right' }, { label: '좌측 하단', value: 'bottom-left' }, { label: '우측 하단', value: 'bottom-right' }] }, { key: 'overlayColor', label: '오버레이 색상', type: 'text' }, { key: 'overlayOpacity', label: '오버레이 투명도 (%)', type: 'number' }, { key: 'borderColor', label: '테두리 색상', type: 'text' }, { key: 'borderWidth', label: '테두리 두께 (px)', type: 'number' }, { key: 'borderRadius', label: '모서리 둥글기 (px)', type: 'number' }, { key: 'maxWidth', label: '최대 너비', type: 'select', options: [{ label: '7xl (기본)', value: '7xl' }, { label: '5xl (좁게)', value: '5xl' }, { label: '전체', value: 'full' }] }, { key: 'linkUrl', label: '링크 URL', type: 'url' }, { key: 'linkTarget', label: '링크 타겟', type: 'select', options: [{ label: '현재 창', value: '_self' }, { label: '새 창', value: '_blank' }] }] },
  ...
  { type: 'page_subnav', label: '페이지 헤더 + 서브메뉴', category: '레이아웃', icon: '🧭', nature: 'dynamic', description: '브레드크럼 + 제목 + 상위 메뉴에 연결된 하위 메뉴(현재 페이지 밑줄). 메뉴에서 자동 생성', variants: [], defaultProps: { title: '', parentMenu: '', breadcrumbHome: '홈' }, editableFields: [{ key: 'title', label: '제목(비우면 상위 메뉴명)', type: 'text' }, { key: 'parentMenu', label: '연결할 상위 메뉴 (슬러그/이름 · 비우면 현재 페이지 기준 자동)', type: 'text' }, { key: 'breadcrumbHome', label: '브레드크럼 홈 라벨', type: 'text' }] },
  ...
  { type: 'newcomer_info', label: '새가족 안내 (레거시)', category: '교회 정보', icon: '🤝', nature: 'static', description: '새가족 환영 메시지', variants: [], defaultProps: { title: '처음 오신 분들을 환영합니다' }, editableFields: [{ key: 'title', label: '제목', type: 'text' }, { key: 'subtitle', label: '부제목', type: 'text' }, { key: 'content', label: '내용', type: 'richtext' }, { key: 'imageUrl', label: '이미지', type: 'image' }] },
  ...
  { type: 'image_gallery', label: '이미지 갤러리', category: '레거시', icon: '🎨', nature: 'static', description: '이미지 목록', variants: [{ id: 'grid', label: '그리드' }, { id: 'masonry', label: '메이슨리' }], defaultProps: { images: [] }, editableFields: [{ key: 'title', label: '제목', type: 'text' }, { key: 'images', label: '이미지', type: 'images', max: 20 }] },
  { type: 'quick_links', label: '바로가기', category: '소개', icon: '🔗', nature: 'static', description: '제목 + 화살표 링크 목록 (기도 요청 →, 소그룹 신청 → 등). 타일 변형 = 아이콘+라벨 그리드', variants: [{ id: 'card', label: '목록 카드' }, { id: 'tiles', label: '아이콘 타일' }], defaultProps: { title: '바로가기', items: [] }, editableFields: [{ key: 'title', label: '제목', type: 'text' }, { key: 'items', label: '링크 (제목=라벨, 내용=URL)', type: 'cardItems', withImage: false }] },
];
    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-xl w-full max-w-2xl max-h-[70vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        {loading ? (
          ...
          <>
            <div className="flex flex-wrap gap-1.5 px-5 py-2 border-b">
              {categories.map((cat) => (
                <button
                  key={cat}
...
export default function PageEditor() {
  ...
  return (
    <div className="flex h-[calc(100vh-8rem)]">
      ...
      {/* Main Board */}
      <div className="flex-1 overflow-y-auto bg-gray-100/50 p-3 sm:p-5"
