...
import type { UpdatePricingInput, UpdateSetupPricingInput } from './schema.js';

const TABLE = 'public.plan_pricing';
const SETUP_TABLE = 'public.setup_pricing';
...
export async function listSetupPricing(activeOnly = false) {
  return prisma.$queryRawUnsafe<Record<string, unknown>[]>(
    `SELECT * FROM ${SETUP_TABLE} ${activeOnly ? 'WHERE is_active = true' : ''} ORDER BY sort_order ASC`,
  );
}

export async function updateSetupPricing(scopeKey: string, input: UpdateSetupPricingInput) {
  const map: Record<string, string> = { label: 'label', price: 'price', fromPrice: 'from_price', isActive: 'is_active' };
  const set: string[] = [];
  ...
  if (set.length === 0) {
    const rows = await prisma.$queryRawUnsafe<Record<string, unknown>[]>(`SELECT * FROM ${SETUP_TABLE} WHERE scope_key = $1`, scopeKey);
    return rows[0] ?? null;
  }
  set.push('updated_at = NOW()');
  const rows = await prisma.$queryRawUnsafe<Record<string, unknown>[]>(
    `UPDATE ${SETUP_TABLE} SET ${set.join(', ')} WHERE scope_key = $${i} RETURNING *`,
    ...values, scopeKey,
  );
  return rows[0] ?? null;
}
