...
// Super-admin only — bulk YouTube import is an operator tool, not a per-tenant
// self-serve action. Tenant context still comes from X-Tenant-Slug (getSchema).
export async function youtubeImportRoutes(app: FastifyInstance) {
  // Is the feature wired (YOUTUBE_API_KEY present)?
  app.get('/youtube-import/status', { preHandler: [requireSuperAdmin] }, async (_request, reply) => {
  ...
  app.post('/youtube-import/sources', { preHandler: [requireSuperAdmin] }, async (request, reply) => {
  ...
  app.post('/youtube-import/fetch', { preHandler: [requireSuperAdmin] }, async (request, reply) => {
  ...
  app.post('/youtube-import/apply', { preHandler: [requireSuperAdmin] }, async (request, reply) => {
