...
export async function formRoutes(app: FastifyInstance) {
  ...
  // ── Admin inbox ────────────────────────────────────────────────────
  app.get('/form-submissions', { preHandler: [requireAuth, requireFeature('forms')] }, async (request, reply) => {
  ...
  app.get('/form-submissions/:id', { preHandler: [requireAuth, requireFeature('forms')] }, async (request, reply) => {
  ...
  app.put('/form-submissions/:id', { preHandler: [requireAuth, requireFeature('forms')] }, async (request, reply) => {
    const { id } = request.params as { id: string };
  ...
  app.delete('/form-submissions/:id', { preHandler: [requireAuth, requireFeature('forms')] }, async (request, reply) => {
    const { id } = request.params as { id: string };
