      ...
      try {
        await prisma.$executeRawUnsafe(`
          CREATE TABLE IF NOT EXISTS "${schema}".newcomer_registrations (
            ...
            "memo"           TEXT,
            "scan_image_url" VARCHAR(1000),
            "created_at"     TIMESTAMPTZ DEFAULT NOW(),
            "updated_at"     TIMESTAMPTZ DEFAULT NOW()
          )
        `);
        // 종이 신청서 스캔 사진 URL(OCR 자동입력용). 기존 테넌트에도 추가(SCHEMA_VERSION↑로 재실행).
        await prisma.$executeRawUnsafe(
          `ALTER TABLE "${schema}".newcomer_registrations ADD COLUMN IF NOT EXISTS "scan_image_url" VARCHAR(1000)`,
        );
        await prisma.$executeRawUnsafe(
