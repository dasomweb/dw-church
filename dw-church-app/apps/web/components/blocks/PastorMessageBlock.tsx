    ...
    <section className="px-4 py-10 sm:px-6 sm:py-16">
      <div className="mx-auto grid max-w-7xl items-center gap-12 md:grid-cols-2">
        <div className={imageFirst ? 'order-2' : 'order-1'}>
          {title && <h2 className="mb-6 text-3xl font-bold font-heading" style={getElementStyle(props, 'title')}>{title}</h2>}
          {message && (
            <div
              className="prose prose-lg mb-6 whitespace-pre-wrap"
              style={getElementStyle(props, 'message')} dangerouslySetInnerHTML={{ __html: message }}
            />
          )}
