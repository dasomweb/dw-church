...
/**
 * 카드뉴스 덱 목록(클라이언트) — 표지(4:5)를 그리드 또는 홈 가로 레일로 보여 주고,
 * 카테고리 칩으로 거른다. 표지를 누르면 CardNewsViewer(몰입형 뷰어)가 열린다.
 * (덱에 카드가 없고 linkUrl 만 있으면 외부/내부 링크로 이동.) 교회 톤(밝은 배경).
 */
...
export function CardNewsDecksClient({ decks, variant = 'grid' }: { decks: DeckItem[]; variant?: 'grid' | 'rail' }) {
  ...
  const openDeck = (d: DeckItem) => {
    if ((!d.cards || d.cards.length === 0) && d.linkUrl) { router.push(d.linkUrl); return; }
    if (d.cards && d.cards.length) setOpen(d);
    else if (d.linkUrl) router.push(d.linkUrl);
  };

  const Cover = ({ d }: { d: DeckItem }) => (
    <button
      type="button" onClick={() => openDeck(d)}
      className="group flex w-full flex-col overflow-hidden rounded-2xl bg-white text-left shadow-sm ring-1 transition hover:-translate-y-0.5 hover:shadow-md"
    ...
    >
      <div className="relative w-full overflow-hidden bg-gray-100" style={{ aspectRatio: '4 / 5' }}>
        ...
        {d.cards && d.cards.length > 1 && (
          <span className="absolute bottom-2.5 right-2.5 inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-semibold shadow-sm" style={{ background: 'rgba(255,255,255,.94)', color: 'var(--dw-text, #16181d)' }}>
      ...
      {(d.title || d.description) && (
        <div className="px-3 py-3">
          {d.title && <div className="line-clamp-2 text-[14.5px] font-semibold leading-snug" style={{ color: 'var(--dw-text, #16181d)' }}>{d.title}</div>}
