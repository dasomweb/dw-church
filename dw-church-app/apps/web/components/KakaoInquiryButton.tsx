 ...
 */
export default function KakaoInquiryButton({ className }: { className?: string } = {}) {
  ...
  // 기본: 우하단 플로팅 풍선 버튼 — 단, 모바일에서는 숨기고 데스크톱에서만 노출
  // (대표님 2026-09-07: 모바일에선 풍선 대신 하단바 50/50에 넣음). className 을
  // 넘기면 그대로 써서 인라인(하단바 half 등)으로 렌더한다.
  const cls = className
    ?? 'fixed bottom-5 right-5 z-50 hidden items-center gap-2 rounded-full bg-[#FEE500] px-5 py-3 text-sm font-bold text-[#191600] shadow-lg transition hover:brightness-95 lg:flex';
