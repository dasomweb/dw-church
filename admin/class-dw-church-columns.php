    ...
    public function dasom_church_banner_column_content($column, $post_id) {
        switch ($column) {
            case 'menu_order':
                ...
                echo '<input type="number" class="banner-menu-order-input" data-post-id="' . esc_attr($post_id) . '" value="' . esc_attr($menu_order) . '" min="0" step="1" style="width:60px;text-align:center;" />';
                break;
            ...
            $(document).ready(function() {
                ...
                $(document).on('input change', '.banner-menu-order-input', function() {
                    ...
                    $.ajax({
                        ...
                        type: 'POST',
                        data: {
                            action: 'dasom_church_update_banner_order',
                            post_id: postId,
                            menu_order: newOrder,
...
// AJAX handler for banner order update
add_action('wp_ajax_dasom_church_update_banner_order', 'dasom_church_update_banner_order_callback');
function dasom_church_update_banner_order_callback() {
