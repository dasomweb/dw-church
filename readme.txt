...
* Fix: Improve server-side JSON parsing with error handling - prevents silent failures when saving album images
* Fix: Ensure album images are properly saved when 15 or fewer images are selected

= 2.62.14 =
* Fix: Remove auto-update custom code that was affecting other plugins in plugins list
* Fix: Album image validation now counts from actual DOM preview instead of hidden input - ensures accurate count after thumbnail deletion
* Fix: Improve form submit validation to prevent saving when 15+ images are selected

= 2.62.13 =
* Fix: Improve folder name fix logic for plugin installation - handle various wrong folder name patterns
* Fix: Prevent album post save when 15+ images are selected - show alert on submit and block save
* Fix: Simplify album image validation - only check on form submit
...
= 2.62.9 =
* Enhancement: Add 15 image limit validation on form submit - prevent saving if more than 15 images are selected
* Fix: Simple validation that blocks submit when album images exceed 15 on publish/update
...
= 2.62.3 =
* Fix: Improve real-time selection count update using requestAnimationFrame for better timing
* Fix: Update count on both checkbox check and uncheck events
* Fix: Ensure count updates immediately on all selection events (add, remove, click, change)
...
* Enhancement: Automatically uncheck excess images beyond 16 limit, keeping first 16 selections in order
* Enhancement: Show automatic warning message when attempting to select more than 16 images
* Enhancement: Maintain selection order and keep only first 16 images when limit is exceeded
...
* Enhancement: Read image count from hidden input field if preview elements are not ready
* Enhancement: Hide "no images" message when images exist
* Enhancement: Improve DOM readiness check with delayed execution
...
* Fix: Improve display_type handling in render function with explicit button template check
...
* Feature: Hierarchical album categories with admin column display
* Feature: REST API support for album categories
...
* Add: 1:1 (Instagram Feed) aspect ratio option
* Add: 4:5 (Instagram Portrait) aspect ratio option
* Update: 9:16 option label to include TikTok and Instagram Story
