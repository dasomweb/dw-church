...
define('DASOM_CHURCH_VERSION', '2.38');
...
function dasom_church_upgrader_pre_download($reply, $package, $upgrader) {
    ...
    if (is_wp_error($response)) {
        return new WP_Error(
            'download_error',
            sprintf(__('???�운로드 ?�류: %s', 'dw-church'), $response->get_error_message())
        );
    }
    ...
    if ($code !== 200) {
        ...
        return new WP_Error(
            'download_failed',
            sprintf(__('???�운로드 ?�패: HTTP %d', 'dw-church'), $code) . '<br>' .
            sprintf(__('URL: %s', 'dw-church'), esc_url($package)) . '<br>' .
            sprintf(__('Token: %s...', 'dw-church'), substr($github_token, 0, 10)) . '<br>' .
            sprintf(__('?�류: %s', 'dw-church'), esc_html(substr($error_message, 0, 200)))
        );
    }
    ...
    if (!$tmpfname) {
        return new WP_Error('temp_file_failed', __('???�시 ?�일 ?�성 ?�패', 'dw-church'));
    }
    
    $body = wp_remote_retrieve_body($response);
    if (file_put_contents($tmpfname, $body) === false) {
        @unlink($tmpfname);
        return new WP_Error('file_write_failed', __('???�일 ?�기 ?�패', 'dw-church'));
    }
...
add_action('admin_init', function() {
    if (isset($_GET['dasom_check_update']) && current_user_can('update_plugins')) {
        ...
        add_action('admin_notices', function() {
            ...
            echo esc_html__('?�데?�트 캐시가 ??��?�었?�니?? ?�러그인 목록???�로고침?�세??', 'dw-church');
            echo '</p></div>';
        });
...
class DW_Church_Management {
    ...
    public function dasom_church_load_textdomain() {
        load_plugin_textdomain('dw-church', false, dirname(plugin_basename(__FILE__)) . '/languages');
    }
    ...
    public function dasom_church_activation() {
        ...
        $default_preacher = __('?�임목사', 'dw-church');
